...
function StaggerText({ text }) {
  return (
    <span aria-label={text} role="text" className="whitespace-nowrap">
      {text.split("").map((ch, i) => (
        <motion.span
          key={i}
          aria-hidden
      ...
      <section className="bg-hero text-hero-text pt-32 pb-20">
        <div className="max-w-6xl mx-auto px-6 grid lg:grid-cols-[1.1fr_0.9fr] gap-14 items-center">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease }}>
            ...
            <h1 className="text-4xl sm:text-5xl md:text-7xl font-extrabold leading-[1.02] tracking-[-0.02em]">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-9">
              <Magnetic className="w-full sm:w-auto">
                <a href={profile.linkedin} target="_blank" rel="noreferrer"
                  className="block text-center px-6 py-3 rounded-lg bg-hero-text text-hero font-semibold hover:opacity-90 transition-opacity">
                  Connect on LinkedIn
                </a>
              </Magnetic>
              <Magnetic className="w-full sm:w-auto">
                <a href={profile.github} target="_blank" rel="noreferrer"
                  className="block text-center px-6 py-3 rounded-lg border border-hero-line font-semibold hover:bg-hero-hover transition-colors">
                  GitHub
                </a>
              </Magnetic>
            </div>
